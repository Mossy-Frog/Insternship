...
chemin<-"C:/Users/maxence.lechemia/Documents/Multi-omics/source transcripto/"
...
net = blockwiseModules(DfComplete, power = 30,
...
#############test 2.b part wgcna#########
  ...
  save(MEs, moduleLabels, moduleColors, geneTree, file = "step2bwgcna.RData")
...
####################Part 2 C of WGCNA###########################
  ...
  lnames = load(file = "C:/Users/maxence.lechemia/Documents/step2bwgcna.RData");
  lnames
  ...
  datTraits[c(1:6,19:24),c(colnames(MetaData))]<-MetaData[1,]
  datTraits[c(7:12),c(colnames(MetaData))]<-MetaData[4,]
  datTraits[c(13:18),c(colnames(MetaData))]<-MetaData[3,]
  datTraits[c(25:30,43:48),c(colnames(MetaData))]<-MetaData[2,]
  datTraits[c(31:36),c(colnames(MetaData))]<-MetaData[6,]
  datTraits[c(37:42),c(colnames(MetaData))]<-MetaData[5,]
  datTraits$name[which(datTraits$name=="A2")]<-1
  datTraits$name[which(datTraits$name=="B1")]<-2
  datTraits$name[which(datTraits$name=="A2-AST2")]<-3
  datTraits$name[which(datTraits$name=="A2-AST17")]<-4
  datTraits$name[which(datTraits$name=="B1-AST2")]<-5
  datTraits$name[which(datTraits$name=="B1-AST2")]<-6
